<template>
    <section :style="{ 'margin-top': marginSize + 'px' }">
        <div class="mainTitle">
            <span>档案移交申请</span>
            <el-button class="checkWay rt" @click="back" v-if="!closeTrue && fanhui">返回</el-button>
            <el-button class="checkWay rt" @click="Tofile" v-if="closeTrue && fanhui">关闭</el-button>
        ...
        <div class="mainDoc">
            <div class="mainCon basicList">
                ...
                <el-row>
                  ...
                  <el-col :span="5"><div class="grid-content bg-purple-light"><span class="span2">{{dateFormat1(totalData.applyDate)}}</span></div></el-col>
                </el-row>
                <el-row>
                ...
                <el-row>
                  ...
                  <el-col :span="21"><div class="grid-content1 bg-purple-light" style="line-height:30px;padding-left:10px;overflow-y:auto;">{{totalData.transferMemo}}</div></el-col>
                </el-row>
                </ul>
            </div>
            <div class="mainCon basicList">
                <div class="clear">
                    ...
                    <el-table border style="width: 100%" stripe max-height="300" :data="listData" highlight-current-row v-loading="listLoading" class="indexTable containTable">
                        <el-table-column  :prop="item.fieldName" :label="item.caption" v-for='(item,index) in tableList' :key="index">
                        </el-table-column>
...
<script lang="ts">
    ...
    export default class appSearch extends Vue{
        ...
        created(){
            this.row = JSON.parse(window.sessionStorage.getItem('TDtransferDetail'));
        ...
        mounted(){
            ...
            let id;
            if(this.processId == 0){
                trans = JSON.parse(window.sessionStorage.getItem('TDtransferDetail'));
                id = trans.id;
            }else{
                id = this.parameOne;
            }

            if(trans != undefined && trans.type){
                this.closeTrue = true;
            }
