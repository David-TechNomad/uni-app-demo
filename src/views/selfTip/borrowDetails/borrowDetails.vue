<template>
    <section>
        <div class="mainTitle">
            ...
            <el-button class="checkWay rt" @click="printPage(row.parentAppNo)" v-show="showPrint">打印</el-button>
        </div>
        <div class="mainDoc">
            ...
            <div class="mainCon basicList">
                <div class="clear">
                    <div class="loanTipTitle">
                        待借阅档案
                        <div class="titleSect"></div>
                        <el-button class="checkWay rt" style="margin:5px 0" @click="change"  v-show="allShow">续借</el-button>
...
<script lang="ts">
    ...
    export default class appSearch extends Vue{
        ...
        back():void{
            let z_check = window.sessionStorage.getItem('z_check');
            if(z_check == 1){
                this['$router'].push({
                    path:'/selfTip/examCheck/peopleCheck',
                });
                window.sessionStorage.removeItem('z_check');
            }else if(z_check == 2){
                this['$router'].push({
            ...
            }else if(window.sessionStorage.getItem('backFile')=='2'){
                this['$router'].push({
                    path:'/selfTip/borrow',
                });
            }else if(window.sessionStorage.getItem('z_check')==3){
                 this['$router'].push({
                     path:'/system/processManagement/detail/processManagementUpdateDetail',
                 });
            }else if(window.sessionStorage.getItem('z_check')==4){
                 this['$router'].go(-1);
            }
        ...
        backBorrowDetails(){
            ...
            window.sessionStorage.setItem('toDetails','1');
            this['$router'].push({
        ...
        electricBorrowDetail(row){
            if(this.row.auditStatus===1||this.row.auditStatus===5){
                Service.waterMarkTime({ appNo:this.applicationNo }).then(res=>{
                    if(res.data.succeeded){
                        if(!res.data.data){
                            let obj = {appNo:this.applicationNo,archiveId:row.id,libId:row.libId,MJ:'1'}
                            window.sessionStorage.setItem('XiangDetail',JSON.stringify(obj));
                            this['$router'].push({
                                path:'/@/views/fileArrange/fileManagement/infile/XiangDetail',
                            })
                        }else{
                            let obj = {appNo:this.applicationNo,archiveId:row.id,libId:row.libId,MJ:row.MJ}
        ...
        change(){
            ...
            if(arr.length===this.list.length){
                window.sessionStorage.setItem('borrowToData', JSON.stringify(this.row));
                if(this.list.length == 0){
                    this.list = this.dataDetail;
                }
                window.sessionStorage.setItem('list', JSON.stringify(this.list));
                this['$router'].push({
                    path:'/selfTip/borrowAgine/changeBorrowPeople1',
                });
            }else{
                this['$message']({
        ...
        printPage(a){
            ...
            Service.printProcessUrl(params).then((res) =>{
                if (res) {
                    let csvData = new Blob([res],{type: 'application/pdf;charset=UTF-8'});
                    if (window.navigator && window.navigator.msSaveOrOpenBlob) {
                        window.navigator.msSaveOrOpenBlob(csvData,'document.pdf');
                    }else{
                        let url = window["URL"].createObjectURL(new Blob([res],{type: 'application/pdf;charset=UTF-8'}));
                        window.open(url);
                    }
                }else{
                    this['$message']({
        ...
        created(){
            //打印隐藏
            if(window.sessionStorage.getItem('z_check')==3){
