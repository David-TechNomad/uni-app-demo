    ...
    <section :style="{ 'margin-top': marginSize + 'px' }">
        ...
        <div class="mainDoc">
            <div class="mainCon basicList">
                <div class="loanTipTitle">申请单号：
                    <b>{{AllData.transferLookup.applicationNo}}</b>
                ...
                <el-form :model="basicForm" class="basicForm topParamsForm fileApplyForm validForm" label-width="120px" :rules="basicFormRules" ref="basicForm">
                    <div class="formItem topTitle">
                        <el-form-item label="标题：" ref="title" prop="title">
                            <el-input class='checkName percentName' v-model.trim="basicForm.title" maxlength="50" placeholder="最多填写1000个字"></el-input>
                        </el-form-item>
                    </div>
                    ...
                    <div class="clear formItem evenTitle whiteTitle">
                        <el-form-item label="接收单位：" ref="danwei" class="lt" prop="danwei">
                            <el-input class="checkName percentName" readonly v-model="basicForm.danwei" :title="basicForm.danwei"></el-input>
                            <el-button class="checkSearch choosePerson thisInp" @click="nextChangeDanwei" style="position: absolute;top: 5px;right: 6px;"><i class="fa fa-search"></i></el-button>
                        </el-form-item>
                        <el-form-item label="接收部门：" ref="bumen" class="lt basicItem" prop="bumen">
                            <el-input class="checkName percentName" readonly v-model="basicForm.bumen"></el-input>
                        </el-form-item>
                    </div>
                    ...
                    <div class="evenTitle">
                        <el-form-item class="things" label="移交说明：" ref="things"  prop="things">
                            <el-input type="textarea" class="checkName percentName" v-model.trim="basicForm.things" placeholder="最多填写1000个字" maxlength="1000"></el-input>
                        </el-form-item>
                    </div>
                </el-form>
            </div>
            <div class="mainCon">
                ...
                <div class="borderTable">
                    <el-table border tooltip-effect="dark" stripe max-height="300" :data="listData" highlight-current-row v-loading="listLoading" @selection-change="selsChange" class="indexTable containTable expandTable">
                         ...
                         <el-table-column  :prop="item.fieldName" :key="item.fieldName" :label="item.caption" v-for='item in tableList'>
                        </el-table-column>
                ...
                <el-form :model="auditDisplay" class="basicForm topParamsForm fileApplyForm" label-width="120px" ref="auditDisplay" :rules="auditDisplayRule">
                    <div class="formItem topTitle">
                        <el-form-item class="checkThings" label="选择审批人：" ref="auditIds" prop="auditIds">
                            <el-input class="checkName percentName" :class="{checkProcess:!checkProcessTrue}" v-model="auditDisplay.auditIds" readonly='readonly' :placeholder="checkProcess"></el-input>
                    ...
                    <div class="topTitle">
                        <el-form-item class="checkThings" label="签字：" ref="Signature" prop="Signature">
                ...
                <el-row style="height:100%">
                    ...
                    <el-col :span="11" style="height:48%;margin-top:1%;">
                        <div class="checkArea checkBottom checkBottomRight">
                            <el-table border tooltip-effect="dark" @row-click="leafClickRow" @row-dblclick="celldblClick" stripe  min-height="176" @selection-change="selsChange1" :data="dataCompany" v-loading="listLoading" class="indexTable containTable">
            ...
            <div class="addTablesearch">
                <div class="addTablesearchForm">
                    <el-form :model="searchForm">
                        ...
                        <el-form-item>
                            <el-input class="checkName2" :title='searchkeyOne.key' v-model="searchkeyOne.key" @keyup.enter.native="beginSearch" style="width:22%;float:left;margin-right:20px"></el-input>
...
<script lang="ts">
    ...
    import Service from './kuaChangeDetails.service';
    ...
    export default class appSearch extends Vue{
        ...
        rank :number;
        fanhui:boolean;
        wenJianFieldMappings : any = [];
        anJuanFieldMappings : any = [];
        deleteSrc : number;
        listData :  any = [];
        ...
        oceaAcceptFondId :number;//之前接收人id
        active : number;
        ...
        back():void{
            this['$router'].go(-1);
            // if(window.sessionStorage.getItem('z_sRouterType') == "r1"){
            //     this['$router'].push({
            //             path:'/selfTip/examination',
            //     })
            //     window.sessionStorage.removeItem('z_sRouterType');
            // }else{
            //     this['$router'].push({
        ...
        getRowClass1(row: any = {}) {
            if (row.row.ARCHIVE_STATUS == 2) {//跨全宗移交状态-->正在移交中
                return 'row-yijiao';
            }
        }
        selsChange(list:any = {}){
        ...
        closeBtns(){//确认关闭
            ...
            //       type: 'warning'
            //   }).then(() => {
            //       Service.closeFileTransfer({"transferId":JSON.parse(window.sessionStorage.getItem('kuaChangeDetails'))}).then(res=>{
        ...
        centerDialogVisibejobEvent(){
            let transferId;
            if(this.processId == 0){
                transferId = JSON.parse(window.sessionStorage.getItem('kuaChangeDetails'));
            }else{
                transferId = this.parameOne;
            }
                Service.closeFileTransfer({"transferId":transferId,closeSign:this.titleNode}).then(res=>{
                        if(res.data.succeeded == true){
                            this.centerDialogVisibejob = false;
                        // this['$router'].push({
                        //     path:'/selfTip/transfer'
        ...
        // 点击选择审批人
        choosePerson():void{
            Service.checkProcess({fondId:this.acceptFondId,transferType:2}).then(res=>{
                if(res.data.succeeded ==true){
        ...
        nameList(){
            ...
            Service.GetSelectRole({
                ...
                if(res.data.succeeded){
                    ...
                    for(var ind of data){
                        if(ind.userName){
                            this.dataCompany.push({ leafName:ind.name,name:ind.userName,select:ind.select,userID:ind.userId })
                        }else{
                            this.dataCompany.push({ leafName:ind.name,name:'',select:ind.select,userID:ind.userId})
                        }
        ...
        leftToRight(){
            ...
            console.log( this.oaClickRowName)
                ...
                if(valid){
                     this.loadingInstance = Loading.service(
                        ...
                        if(this.processId == 0){
                            ...
                            // this.transferType = JSON.parse(window.sessionStorage.getItem('kuaChangeTransFortype'))
                             if(window.sessionStorage.getItem('kuaChangeDetails')){
                                transferId = JSON.parse(window.sessionStorage.getItem('kuaChangeDetails'));
                                this.transferType = JSON.parse(window.sessionStorage.getItem('kuaChangeTransFortype'))
                            }else{
                                transferId = JSON.parse(window.sessionStorage.getItem('kuaTransforDetails')).transferId;
                                this.transferType = JSON.parse(window.sessionStorage.getItem('kuaTransforDetails')).transferType;
                            }
                        }else{
                            transferId = this.parameOne;
                            ...
                            this.$nextTick(() => { this.loadingInstance.close();});
                              ...
                              if(idsArr.length==0){
                                ...
                                Service.reissueFileTransfer({
                                    ...
                                    targetFondId:this.targetFondId,
                                    targetKindId : this.targetFondId,
                                    deleteSrc : this.deleteSrc,
                                    wenJianFieldMappings : this.wenJianFieldMappings,
                                    anJuanFieldMappings : this.anJuanFieldMappings,
                                    acceptDept : this.basicForm.bumen
                                }).then(res=>{
                                    if(res.data.succeeded){
                                        this['$confirm']('跨全宗移交申请发起成功。', '提示', {
                                            ...
                                            // this['$router'].go(-1)
                                            if(this.processId == 0){
                                                let z_sRouterType = window.sessionStorage.getItem('z_sRouterType')
                                                if(z_sRouterType == 'r1'){
                                                    this['$router'].push({
                                                        path:'/selfTip/examination'
                                                    })
                                                window.sessionStorage.removeItem('z_sRouterType');
                                                    ...
                                                    var oaFlag = JSON.parse(window.sessionStorage.getItem('OAClose')).flag;
                                                    if(oaFlag){
                                                        window.close();
                                                    }else{
                                                        this['$router'].push({
    ...
    // 双击抄送常联系人添加
        ...
        FileSearch(params:any){
            if(this.rank == 3){//档案级
                Service.FileSearch4(params).then(res=>{
                    if(res.data.succeeded){
            ...
            }else if(this.rank == 4){//文件级
                Service.WenjianSearch4(params).then(res=>{
                    if(res.data.succeeded){
                        this.searchData=res.data.data;
                        this.page = res.data.page;
                        this.total = res.data.totalRecords;
                ...
                for(var ind of this.selection){
                    if(ind.ARCHIVE_STATUS==1){
                        ...
                        if(i==0){
                            ...
                            window.sessionStorage.setItem('getAddFileList1',JSON.stringify(obj));
        ...
        nextChangeDanwei(){//选择接收单位
            var obj = {id:JSON.parse(window.sessionStorage.getItem('kuaChangeDetails')),status:1,kindId:this.kindId,active:this.active,transferId:this.nextTransferId,transferType:this.transferType}
             window.sessionStorage.setItem('kuaTransforDetails',JSON.stringify(obj))
             var objs={
                 list:this.listData,
                 libId:this.libId
             }
             var bace = {
                 title:this.basicForm.title,
                 things:this.basicForm.things,
                 Signature:this.auditDisplay.Signature
             }
             window.sessionStorage.setItem('getAddFileList1',JSON.stringify(objs))
             window.sessionStorage.setItem('basicMeg',JSON.stringify(bace))
                this['$router'].push({
                    path:'/views/selfTip/kuaTransforDetails/kuaTransforDetails',
                    name:'相关档案的跨全宗移交'
                });
        }
        handleClose1(){
        ...
        created(){
            ...
            if(url.indexOf("transferId") > 0){
                ...
                window.sessionStorage.setItem('OAClose',JSON.stringify(OAflag))
            }else{
                this.processId = 0;
                this.fanhui = true;
            }
            this.getUserInfo({})
                 ...
                 if(this.processId == 0){
                        if(window.sessionStorage.getItem('kuaChangeDetails')){
                            ...
                            transfer = JSON.parse(window.sessionStorage.getItem('kuaTransforDetails')).transferId;
                            this.transferType = JSON.parse(window.sessionStorage.getItem('kuaTransforDetails')).transferType;
                        }
                       
                    }else{
                        transfer = this.parameOne;
                        this.transferType = this.parameTow;
                        
                    }  
                this.nextTransferId = transfer;
                Service.fileTransfer({
                    ...
                    this.active= res.data.data.rank;
                    this.libId = res.data.data.libId;
                    this.acceptFondId = res.data.data.acceptFondId;
                    this.oceaAcceptFondId = res.data.data.acceptFondId;
                    this.rank = res.data.data.rank;
                    // this.basicForm.danwei = res.data.data.transferLookup.acceptOrg;
                    // this.basicForm.bumen = res.data.data.transferLookup.acceptDept;
                    // var objs= {fond:this.fondId,cy:this.basicForm.danwei,ct:this.basicForm.bumen}
                    // window.sessionStorage.setItem('contrastItem1',JSON.stringify(objs));
                    var obj= {};
                    // if(JSON.parse(window.sessionStorage.getItem('contrastItem1')) != null && JSON.parse(window.sessionStorage.getItem('contrastItem1')) != {}){
                    //     alert(88)
                    //     var jsbm = JSON.parse(window.sessionStorage.getItem('contrastItem1'))
                    //     console.log(jsbm)
                    //     this.targetKindId = jsbm.targetId;
                    //     this.targetFondId = jsbm.targetfond
                    //     this.basicForm.danwei = jsbm.cy;
                    //     this.basicForm.bumen = jsbm.ct;
                    //     this.fondId = jsbm.fond;
                    //     this.deleteSrc = jsbm.active;
                    // }
                    ...
                    if(JSON.parse(window.sessionStorage.getItem('AJmap'))){
                        this.anJuanFieldMappings = JSON.parse(window.sessionStorage.getItem('AJmap'));
                    }
                    if(JSON.parse(window.sessionStorage.getItem('WJmap'))){
                        this.wenJianFieldMappings = JSON.parse(window.sessionStorage.getItem('WJmap'));
                    }
                       
                    if(window.sessionStorage.getItem('getAddFileList1')!="{}"){
                            this.listData = JSON.parse(window.sessionStorage.getItem('getAddFileList1')).list;
                        }else{
                            this.listData = res.data.data.archives;
                        }
                    if(window.sessionStorage.getItem('acceptFondId')!='{}'){
                        if(JSON.parse(window.sessionStorage.getItem('acceptFondId')).targetfond == ''){
                            this.acceptFondId = res.data.data.acceptFondId //点击返回 赋值初始fondId
                        }else{
                             this.acceptFondId = JSON.parse(window.sessionStorage.getItem('acceptFondId')).targetfond;
                        }         
                    }
                     if(window.sessionStorage.getItem('recordTransferApply1')!="{}"){
                            this.acceptFondId = JSON.parse(window.sessionStorage.getItem('recordTransferApply1')).targetfond;
                    }
                    if(window.sessionStorage.getItem('basicMeg')!="{}"){
                        var basicMeg = JSON.parse(window.sessionStorage.getItem('basicMeg'));
                        console.log(basicMeg)
                        this.basicForm.title = basicMeg.title;
                        this.basicForm.things = basicMeg.things;
                        this.auditDisplay.Signature = basicMeg.Signature;
                    }
                    Service.checkProcess({fondId:this.acceptFondId,transferType:2}).then(res=>{
                        if(res.data.succeeded ==true){
                            if(res.data.data == true){
        ...
        beforeRouteEnter(to,from,next){
            next(vm=>{
                if(from.name=='相关档案的跨全宗移交判断字段'){
         
                  if(window.sessionStorage.getItem('contrastItem1') != '{}'){
                       var jsbm = JSON.parse(window.sessionStorage.getItem('contrastItem1'))
                        console.log(jsbm)
                        vm.targetKindId = jsbm.targetId;
                        vm.targetFondId = jsbm.targetfond
                        vm.basicForm.danwei = jsbm.cy;
                        vm.basicForm.bumen = jsbm.ct;
                        if(jsbm.targetStore.indexOf('-案卷库')!=-1){
                            var str = jsbm.targetStore;
                            vm.basicForm.bumen = str.replace('-案卷库','')
                        }
                        //vm.basicForm.bumen = jsbm.targetStore;
                        vm.fondId = jsbm.fond;
                        vm.deleteSrc = jsbm.active;
                        
                    }
                    
                }else if(from.name=='相关档案的跨全宗移交'){
                    if(window.sessionStorage.getItem('recordTransferApply1') != '{}'){
                       var jsbm = JSON.parse(window.sessionStorage.getItem('recordTransferApply1'))
                        console.log(jsbm)
                        vm.targetKindId = jsbm.targetId;
                        vm.targetFondId = jsbm.targetfond
                        vm.basicForm.danwei = jsbm.cy;
                         vm.basicForm.bumen = jsbm.ct;
                        if(jsbm.ct.indexOf('-案卷库')!=-1){
                            var str = jsbm.ct;
                            vm.basicForm.bumen = str.replace('-案卷库','')
                        }
                        //vm.basicForm.bumen = jsbm.targetStore;
                        ...
                        vm.deleteSrc = jsbm.active;
                         ...
                         var transfer;
                          ...
                          Service.fileTransfer({
                                ...
                                window.sessionStorage.setItem('acceptFondId',JSON.stringify(acceptFondId))
                                vm.basicForm.danwei = res.data.data.transferLookup.acceptOrg;
                                vm.basicForm.bumen = res.data.data.transferLookup.acceptDept;
                                vm.acceptFondId = res.data.data.acceptFondId;

                            });
                    }
                    
                }else if(from.name=='移交详情'){
                        if(window.sessionStorage.getItem('getAddFileList1')!='{}'){
                          console.log(window.sessionStorage.getItem('getAddFileList1'))
                            var listData = window.sessionStorage.getItem('getAddFileList1');
                            ...
                            Service.fileTransfer({
                                ...
                                vm.basicForm.bumen = res.data.data.transferLookup.acceptDept;
                                vm.acceptFondId = res.data.data.acceptFondId;
                                vm.rank = res.data.data.rank;
                            

                            });
                    }else{
                          var transfer = JSON.parse(window.sessionStorage.getItem('kuaChangeDetails'));
                            Service.fileTransfer({
                                "transferId":transfer,
                                "transferStatus":1
                            }).then(res=>{
                    ...
                    Service.fileTransfer({
                        ...
                        window.sessionStorage.setItem('recordTransferApply1',JSON.stringify({}));
                        window.sessionStorage.setItem('getAddFileList1',JSON.stringify({}));
                        window.sessionStorage.setItem('basicMeg',JSON.stringify({}))
                        window.sessionStorage.setItem('acceptFondId',JSON.stringify(acceptFondId))
                        vm.basicForm.danwei = res.data.data.transferLookup.acceptOrg;
                        vm.basicForm.bumen = res.data.data.transferLookup.acceptDept;
                        vm.acceptFondId = res.data.data.acceptFondId;
                        vm.rank = res.data.data.rank;
                        vm.listData = res.data.data.archives;
                        vm.basicForm.title = res.data.data.transferLookup.title;
                        vm.basicForm.things = res.data.data.transferLookup.transferMemo;
