...
<script lang="ts">
    ...
    export default class processManagementUpdate extends Vue{
       ...
       //审批人弹框确定
        ...
        //修改提交
        applySubmit(){
            let utilizeProcessList = [];
            for(let child of this.stepMap){
                for(let child1 of this.stepMapAll){
                    if(child.id == child1.id && child.auditUserId != child1.auditUserId){
                        let obj = {
                            id:child.id,
                            auditUserId:child.auditUserId
                        };
                        utilizeProcessList.push(obj);
                    }
                }
            };
            let utilizeProcessRecordList = [];
            for(let child of this.recordData){
                let obj = {
                    id:child.id,
                    auditRemark:child.auditRemark
                };
                utilizeProcessRecordList.push(obj);
            }
            let parames = {
                applicationNo:this.row.processNo,
                utilizeProcessList:utilizeProcessList,
                utilizeProcessRecordList:utilizeProcessRecordList
            };
            Service.updateUtilizeProcess(parames).then((res) =>{
                if(res.data.succeeded){
                    this['$message']({
                        message: "修改成功！",
                        type: 'success'
