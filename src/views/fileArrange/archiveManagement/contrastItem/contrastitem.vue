<template>
    <section class="contrastitem">
        ...
        <div class="tableheated" style="width: 55%;float: left; background: #fff;margin-left:10px;">
            <div class="sorts" style="margin:10px 0; padding-left:5px;">
                <span>要移动的目录</span>
            ...
            <div class="enterstoreTables" >
                <!--档案表格-->
                <el-row class="fileList" style="padding:0 5px 5px;">
                    <el-col :span="24">
                        <el-table
                                :data="fileList"
                                style="width:100%"
        ...
        <div class="raw" style="width:43%;float:right;background:#fff" v-if="rawSHow">
            <div class="duiying">
                <span>{{active==1?'著录项对应':'案卷著录项对应'}}</span>
                <el-button class="checkWay" style="float:right;" v-if="active==1" @click="allOKB">完成</el-button>
                <el-button class="checkWay" style="float:right;margin-right:10px;" v-if="active==0" @click="rawshowA">下一步</el-button>
                <el-button class="checkWay" style="float:right;margin-right: 8px;" @click="goBack">上一步</el-button>
            </div>
            <div class="left">
                <span class="Titlespan">原档案库</span>
                <div class="searchCompany">{{selectValueY}}</div>
                <div class="contrastreeTitle contrastTitle">
            ...
            <div class="right">
                <span class="Titlespan">目标档案库</span>
...
<script lang="ts">
    ...
    import Service from './contrastItem.service'
    @Component
    export default class contrastitem extends Vue{
        ...
        resetY(){
            var str = window.sessionStorage.getItem('srcFields');
            this.formerList = JSON.parse(str);
            this.selectIndexY=0;
        };
        resetM(){
            var str1 = window.sessionStorage.getItem('targetFields');
            this.targetList = JSON.parse(str1);
            this.selectIndexM=0;
        };
        resetYB(){
            var str = window.sessionStorage.getItem('WJSrcFields');
            this.formerListB = JSON.parse(str);
            this.selectIndexYB=0;
        };
        resetMB(){
            var str1 = window.sessionStorage.getItem('WJTargetFields');
            this.targetListB = JSON.parse(str1);
            this.selectIndexMB=0;
        };
        created(){
            var str = window.sessionStorage.getItem('contrastItem');
...
@import '../../../../styles/archiveManage/contrastStyle.scss';
</style>