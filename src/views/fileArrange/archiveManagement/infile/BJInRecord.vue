                    ...
                    <div class="div1 scrollFiles">
                        <div class="pImg" v-for="(item,index) in imgList" :key="index">
                            <div class="ImgClass" :class='{selectPdfColor:index==nowIndex}'>
                                <img @click="deletePre(item.guid,index,item.title,item.extension,item.convertFlag)" v-if="z_extension(item.extension)&&item.thumbnailFlag" @dblclick="open(item.guid,item.extension)" :src="urlImg+'/file/archive/downloadThumbnail?originalGuid='+item.guid+'&preview=false'" alt="">
                                <img @click="deletePre(item.guid,index,item.title,item.extension,item.convertFlag)" v-if="z_extension1(item.extension)&&item.convertFlag&&item.thumbnailFlag" @dblclick="open(item.guid,item.extension)" :src="urlImg+'/file/archive/downloadThumbnail?originalGuid='+item.guid+'&preview=false'" alt="">
                                <img @click="deletePre(item.guid,index,item.title,item.extension,item.convertFlag)" v-if="z_extension(item.extension)&&!item.thumbnailFlag" src="../../../../imgs/noPic.jpg" alt="" @dblclick="open(item.guid,item.extension)">
                                ...
                                <img @click="deletePre(item.guid,index,item.title,item.extension,item.convertFlag)" v-if="z_extension1(item.extension)&&item.thumbnailFlag&&!item.convertFlag" :src="urlImg+'/file/archive/downloadThumbnail?originalGuid='+item.guid+'&preview=false'" alt="" @dblclick="open(item.guid,item.extension)">
                                <img @click="deletePre(item.guid,index,item.title,item.extension,item.convertFlag)" v-if="z_extension1(item.extension)&&!item.thumbnailFlag&&item.convertFlag" src="../../../../imgs/noPic.jpg" alt="" @dblclick="open(item.guid,item.extension)">
                            </div>
...
<script lang="ts">
    ...
    import Service from './BJInRecord.service';
    ...
    export default class appfileManagement extends Vue{
        ...
        back():void{
            this['$router'].push({
                path:'/fileArrange/archiveManagement/archiveIn',
            });
        };
        //编辑之后的提交
        Editcommit(form,cb){
                ...
                if(valid){
                    ...
                    Service.SaveFile({recomputeDh:true,fondId:this.fondId,libId:this.bids,data:this.addfile}).then(res=>{
                        if(res.data.succeeded){
                            ...
                            this.keepRight = true;
                            window.sessionStorage.setItem('DBJres',JSON.stringify(res.data.data));
                            if(this.keepRight){
                                this.addFileName = '';
                                this.fileSingle = {};
                                this.Suces("保存成功。");
        ...
        /** 断点续传集合 END*/
        mounted(){
            var str = window.sessionStorage.getItem('BJInRecord');
