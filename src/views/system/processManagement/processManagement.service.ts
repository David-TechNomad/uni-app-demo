...
// 档案借阅保存
function updateUtilizeProcess(params){
    let url = ApiConfig.updateUtilizeProcess;
    return HttpRequest.requestJsonPost(url,params)
};

// 强制关闭（全宗）
function closeFondWorkFlow(params){
    let url = ApiConfig.closeFondWorkFlow;
    return HttpRequest.requestJsonPost(url,params)
};

// 强制通过（全宗）
function toEndFondWorkFlow(params){
    let url = ApiConfig.toEndFondWorkFlow;
    return HttpRequest.requestJsonPost(url,params)
};

// 强制驳回（全宗）
function toStartFondWorkFlow(params){
    let url = ApiConfig.toStartFondWorkFlow;
    return HttpRequest.requestJsonPost(url,params)
};

// 全宗修改
function updateFondWorkFlow(params){
    let url = ApiConfig.updateFondWorkFlow;
    return HttpRequest.requestJsonPost(url,params)
};
...
// 移交保存
function fileUpdateProcess(option) {
    let url = ApiConfig.fileUpdateProcess;
    return HttpRequest.requestJsonPost(url,option)
}

// 移交关闭
function fileEnforceCloseProcess(option) {
    let url = ApiConfig.fileEnforceCloseProcess;
    return HttpRequest.requestJsonPost(url,option)
}

// 移交驳回或通过
function fileEnforcePassProcess(option) {
    let url = ApiConfig.fileEnforcePassProcess;
    return HttpRequest.requestJsonPost(url,option)
}
