<template>
            <el-table
              ...
              <el-table-column  prop="name" label="操作" width="200" v-if="tableTitle.length>0">
                    <template slot-scope="scope">                                                 
                        <el-button class="tabletoolBtn" @click="handSelected(scope.row)" v-if="activeName=='file'">查看卷内文件</el-button>
                        <el-button class="tabletoolBtn" v-if="activeName=='file'" @click="releaseTable()">整编发布</el-button>                      
                    </template>                 
              </el-table-column>  
              ...
              <!-- <el-table-column prop="" label="档号">
                   <template slot-scope="scope">                                             
                    </template>
              </el-table-column> 
                <el-table-column prop="" label="题名">
                   <template slot-scope="scope">                                             
                    </template>
              </el-table-column>
              <el-table-column prop="" label="江西省民生档案共享利用平台">
                   <template slot-scope="scope">                                             
                    </template>
              </el-table-column>
              <el-table-column prop="" label="江西省数字档案共享利用平台">
                   <template slot-scope="scope">                                             
                    </template>
              </el-table-column>             -->
          </el-table>
...
<script lang="ts">
    ...
    import Service from './releaseTable.service';
    import {
        State,
        Getter,
        Action,
    ...
    export default class releaseTable extends Vue{  
        ...
        getTable(val){                          
            ...
            Service. getArchiveList(params).then(res=>{
                  if(res.data && res.data.succeeded){ 
                    ...
                    this.$nextTick(()=>{
                        setTimeout(()=>{
                            that.AimTable.scrollTop = that.BackScroll;
                            var index = null;
        ...
        mounted(){
            ...
            this.AimTable.addEventListener('scroll',()=>{ 
               if(this.activeName!="Ingroupfile"){
