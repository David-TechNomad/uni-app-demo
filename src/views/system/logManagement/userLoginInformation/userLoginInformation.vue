        ...
        <div class="searchKey boxshow contentbox-full">           
              <el-form :inline="true" >
                <el-form-item label="登入IP：">
                    <el-input placeholder="单行输入" v-model="Gfilters.ipAddress" size="small"></el-input>
                </el-form-item> 
                <el-form-item class="" label="登入时间" ref="year">
                    <el-date-picker size="small" type="date" ref="startDate" placeholder="开始日期" class="checkName" v-model="Gfilters.preLoginDateTime"></el-date-picker>
                    <span style="margin:0 10px">至</span>
                    <el-date-picker size="small" type="date" class="checkName" placeholder="结束日期" v-model="Gfilters.endLoginDateTime" ref="endDate" :picker-options="pickerOptions"></el-date-picker>
                </el-form-item>            
                <el-form-item>
                    <el-button class="defaultBtn" @click="findUser">查询</el-button> 
                </el-form-item>
             </el-form>
        </div>
                ...
                <div class="ta-r">     
                    <el-button class="defaultBtn" @click="exportData()">导出</el-button>
                                   ...
                                   <el-table
                                        ...
                                        <el-table-column prop="ipAddress" label="登录IP" align="left">                                           
                                        </el-table-column>
                                        <el-table-column prop="type" label="登录类型" align="left">                                           
                                        </el-table-column>
                                        <el-table-column prop="machineName" label="机器名" align="left">                                           
                                        </el-table-column>
                                        <el-table-column prop="loginDateTime" label="登入时间" align="left">
                                            <template slot-scope="scope">
                                                {{datatime(scope.row.loginDateTime)}}
                                            </template>                                           
                                        </el-table-column>
                                        <el-table-column prop="logoutDateTime" label="登出时间" align="left">
                                            <template slot-scope="scope">
                                                {{datatime(scope.row.logoutDateTime)}}
                                            </template>                                           
                                        </el-table-column>
                                    </el-table>  
...
<script lang="ts">
  ...
  import Service from './userLoginInformation.service';
   @Component
    export default class appSuperSearch extends Vue{  
        ...
        constructor(){
            ...
            this.Gfilters = {  
                ipAddress:null,
                preLoginDateTime:null,
                endLoginDateTime:null, 
                page: this.page,
        ...
        dealDisabledDate (time) {
            return time.getTime() < this.Gfilters.preLoginDateTime 
        };
        ...
        findUser(){
            Service.getLoginLogList(this.Gfilters).then(res=>{
...
            var d = new Date(time);
        ...
        exportData(){
            Service.loginLogExport({
                "idStr":this.chooseIdS.join(","),
                "ipAddress":this.Gfilters.ipAddress,
                "preLoginDateTime":this.Gfilters.preLoginDateTime,
                "endLoginDateTime":this.Gfilters.endLoginDateTime,
                ...
                link.href = url;
                link.setAttribute('download', '用户登录日志.xlsx');
