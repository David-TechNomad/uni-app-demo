<template>
    <section class="wrapprSection">
        ...
        <div class="">
                ...
                <div class="pd10 boxshow bgwhite mt10 batchMountTaskView" v-show="DaoRu">
                    <div class="">
                        <el-table
                            :data="resetData"
                            border
                            ...
                            <el-table-column  prop="name" label="操作"  width="150">
                                    <template slot-scope="scope">                                                 
                                        <el-button class="tabletoolBtn" @click="guaJieDetail(scope.row,scope.row.id)" v-if="scope.row.taskStatus !== 1">详情</el-button>
                                        <el-button class="tabletoolBtn" @click="beginCF(scope.row.id)" v-if="scope.row.taskStatus === 1">启动</el-button>
                                        <el-button class="tabletoolBtn" @click="deleteClick(scope.row.id)" v-if="scope.row.taskStatus !== 2 && scope.row.taskStatus !== 3">删除</el-button> 
                                    </template>
                            </el-table-column>
                            ...
                            <el-table-column prop="matchedArchiveCount" label="匹配档案数量">                                      
                            </el-table-column>
                            <el-table-column prop="taskStatus" label="挂接状态">
                                <template slot-scope="scope">                     
                                    <el-progress v-if="(scope.row.taskStatus === 3 || scope.row.taskStatus === 2) && progressShow" :percentage="scope.row.progress"></el-progress> 
                                    <span v-else-if="mountCode" >挂接完成</span>  
      ...
      <!-- 查看 -->
        <el-dialog class="myDialogCenter" title="批量挂接" :visible.sync="visibleAble" :close-on-click-modal="true">
           <p style="padding:10px 15px;">文件匹配成功{{totalItemsCount}}条, 挂接成功{{succeededItemsCount}}条</p>
...
Component.registerHooks([
 ...
 @Component({
    ...
    // 开始挂接
    beginCF(taskId):void{
        Service.fileUpStart({taskId:taskId}).then(res=>{
