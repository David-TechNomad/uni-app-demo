...
<script lang="ts">
    ...
    import Service from './combineExistingST.service'
    @Component
    export default class combineExistingST extends Vue{
        ...
        keepFile(form){
            this['$refs'][form]['validate']((valid)=>{
                if(valid){
                 ...
                 Service.groupToExistingAnJuan(obj).then(res=>{
                     if(res.data.succeeded){
                         this['$confirm']('组卷成功', '提示', {
                             confirmButtonText: '确定',
                             cancelButtonText: '取消',
                             showCancelButton:false,
                             type: 'info'
                         }).then(()=>{
                             this['$router'].push('/fileArrange/fileManagement')
                         })
                     }else{
        ...
        created(){
            var str = window.sessionStorage.getItem('combineExistingST');
            var obj = JSON.parse(str)
            this.libId =obj.lib/1;
            this.wenId = obj.wenId/1;
            this.kindId = obj.kindId/1;
            this.fondId = obj.parent/1;
