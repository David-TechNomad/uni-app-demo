    ...
    <section>
        ...
        <div class="mainDoc">
            <div class="mainCon basicList">
                ...
                <el-row>
                  <el-col :span="3"><div class="grid-content bg-purple"><span class="span1">权限范围</span></div></el-col>
                  <el-col :span="21"><div class="grid-content bg-purple-light">
                      <span class="span2" v-for="(item,index) in allData.archiveTreeReletions">
                      <span v-if="index == 0"> {{item.foundName}}【 </span><span v-if ="index != 0" > , </span> {{item.archiveTreeName}}</span><b style="position:relative;top:10px;left:15px">】</b>
                        <!-- <span v-if="index == 0"> {{item.foundName}}</span>【 {{item.archiveTreeName}} 】<span v-if ="index != allData.archiveTreeReletions.length-1" > , </span> -->
                       
                 </div></el-col>
                </el-row>
                <el-row>
                  <el-col :span="3"><div class="grid-content1 bg-purple" style="border-left:1px solid #ccc;border-right:0"><span class="span1">申请事由</span></div></el-col>
                  <el-col :span="21"><div class="grid-content1 bg-purple-light" style="padding-left:10px;line-height:30px;overflow-y:scroll">{{allData.archivist.applyReason}}</div></el-col>
                </el-row>
                </ul>
            </div>
            <div class="mainCon basicList">
    ...
    export default class appSearch extends Vue{
        ...
        mounted(){
            ...
            Service.getRecordById({
                id:JSON.parse(window.sessionStorage.getItem('SDchaneShenTransDetail')).toString()
            }).then(res=>{
