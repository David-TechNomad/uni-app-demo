<template>
    <section>
        ...
        <div class="mainDoc">
            ...
            <div class="mainCon basicList">
                ...
                <el-form :model="basicForm2" class="basicForm topParamsForm fileApplyForm" label-width="140px" ref="basicForm2">
                    <div class="formItem topTitle">
                        <el-form-item class="checkThings validposition" label="依次选择审批人："  prop="auditIds" :rules="[{required: true, message: '请选择', trigger:['change','blur']}]">
                            ...
                            <!--<span class="noSubmit">{{checkProcess}}</span>-->
                    ...
                    <div class="formItem evenTitle whiteTitle">
                        <el-form-item class="checkThings validposition" label="选择抄送人：">
        ...
        <el-dialog class="checkModal" title="选择抄送人" :visible.sync="writorVisible" :close-on-click-modal="true" :modal-append-to-body="false">
            <div class="checkCon">
                <el-row style="height:100%">
                    ...
                    <el-col :span="11" style="height:48%;margin-top:1%;">
                        <div class="checkArea checkBottom checkBottomLeft">
                            <el-table border tooltip-effect="dark" @row-click="oaClickRow" :height = "oaHeight" stripe :data="dataOa" highlight-current-row v-loading="comLoading" class="indexTable containTable" border>
                                <el-table-column :show-overflow-tooltip="true" prop="fullName" label="姓名">
                                </el-table-column>
                                <el-table-column :show-overflow-tooltip="true" prop="unitname" label="所属部门">
                                </el-table-column>
                    ...
                    <el-col :span="11" style="height:48%;margin-top:1%;">
                        <div class="checkArea checkBottom checkBottomRight">
                            <el-table border border tooltip-effect="dark" @row-click="leafClickRow" @row-dblclick="cellChaodblClick" :height = "oaHeight" stripe :data="dataChao" highlight-current-row class="indexTable containTable">
                                <el-table-column :show-overflow-tooltip="true" prop="fullName" label="姓名">
                                </el-table-column>
                                <el-table-column :show-overflow-tooltip="true" prop="unitname" label="所属部门">
                                </el-table-column>
        ...
        <el-dialog class="checkModal fileTransferApplyAdd" title="选择需要移交的档案" :show-close="true" :visible.sync="tableVisible" :modal-append-to-body="false">
            ...
            <di class="tableContainer">
                <el-table border tooltip-effect="dark" :stripe="true" :height="YJHeight" :data="searchData" :highlight-current-row="false" ref="searchTable" class="indexTable containTable expandTable" @selection-change="selectionChange2" :row-class-name="getRowClass">
                    <el-table-column type="selection" width="30" label="#">
                    </el-table-column>
                    <template v-for="item in tableArrB">
                        <el-table-column :prop="item.dictData==null?item.fieldName:item.fieldName+'_Text'" :label="item.caption" :width="item.width" v-if="!item.hidden"></el-table-column>
...
<script lang="ts">
    ...
    import Service from './fileTransferApply.service';
       ...
       // Signature:string;
        ...
        choosePerson():void{
            if(this.checkProcessTrue){
                ...
                // this['$refs']['basicForm2']['resetFields']();
            }
        };
...
// 懒加载树
        ...
        nameList(){
            this.dataCompany = [];
            Service.GetSelectRole({fondId:this.fondId,transferType:1}).then(res=>{
                if(res.data.succeeded){
                    let data = res.data.data;
                    for(var ind of data){
                        if(ind.userName){
        ...
        chooseWritor():void{
            ...
            if(!this.isFirst1){
                ...
                this.loadNode1(this.Treenode1,this.Treeresolve1);
            }
        };
                            ...
                            Service.saveTransferLookup({
                                ...
                                if (res.data.succeeded) {
                                    ...
                                    this['$confirm']('文件移交申请发起成功', '提示', {
                                        ...
                                        window.sessionStorage.setItem('TDtransferDetail',JSON.stringify(obj))
                                        this['$router'].push({
                                            path:'/selfTip/transferDetail/transforDetail',
                                        });
                                    });
                                } else {
                                    this.getError(res.data.errorMessage);
    ...
    // 双击抄送常联系人添加
        ...
        handleCurrentChange(val) {
            ...
            if(this.type==1){
                ...
                this.WenjianSearch({kindId:this.kindId,fondId:this.fondId,searchKey:this.searchValue,pageCondition:{page:this.page, pageSize:this.limit,sort:""}});
            }
        ...
        beginSearch(){
          ...
          //     this.searchArr = arr;
            ...
            if(this.type==1){
                ...
                this.WenjianSearch({kindId:this.kindId,fondId:this.fondId,searchKey:this.searchValue,pageCondition:{page:1, pageSize:this.limit,sort:""}});
            }
        ...
        created(){
            ...
            var str = window.sessionStorage.getItem('fileTransferApply');
            ...
            // }
            this.getUserInfo({});
            Service.checkProcess({fondId:this.fondId,transferType:1}).then(res=>{
                if(res.data.errorCode==-1){
                    this.checkProcess = res.data.errorMessage;
                    // this.basicForm2.auditIds = this.checkProcess;
